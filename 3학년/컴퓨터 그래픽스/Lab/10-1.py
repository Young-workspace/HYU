...
def exp(rv):
    ...
    u = normalized(rv)
    R = np.identity(3)
    R[0][0] = np.cos(ge) + u[0]*u[0]*(1 - np.cos(ge))
    R[0][1] = u[0]*u[1]*(1 - np.cos(ge)) - u[2]*np.sin(ge)
    R[0][2] = u[0]*u[2]*(1 - np.cos(ge)) + u[1]*np.sin(ge)
    R[1][0] = u[1]*u[0]*(1 - np.cos(ge)) + u[2]*np.sin(ge)
    R[1][1] = np.cos(ge) + u[1]*u[1]*(1 - np.cos(ge))
    R[1][2] = u[1]*u[2]*(1 - np.cos(ge)) - u[0]*np.sin(ge)
    R[2][0] = u[2]*u[0]*(1 - np.cos(ge)) - u[1]*np.sin(ge)
    R[2][1] = u[2]*u[1]*(1 - np.cos(ge)) + u[0]*np.sin(ge)
    R[2][2] = np.cos(ge) + u[2]*u[2]*(1 - np.cos(ge))
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'학번-10-1', None,None)
    if not window:
        glfw.terminate()
        return
