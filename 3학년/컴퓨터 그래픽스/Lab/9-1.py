...
def render():
    ...
    #gluLookAt(5*np.sin(a),np.cos(b),5*np.cos(r), 0,0,0, 0,1,0)
    # draw global frame
    drawFrame()
...
def key_callback(window, key, scancode, action, mods):
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        if key==glfw.KEY_A:     #increase xang by 10 degree
            a += np.radians(10)
        elif key==glfw.KEY_Z:   #decrease xang by 10 degree
            a += np.radians(-10)
        elif key==glfw.KEY_S:   #increase yang by 10 degree
            b += np.radians(10)
        elif key==glfw.KEY_X:   #decrease yang by 10 degree
            b += np.radians(-10)
        elif key==glfw.KEY_D:   #increase zang by 10 degree
            r += np.radians(10)
        elif key==glfw.KEY_C:   #decrease zang by 10 degree
            r += np.radians(-10)
        elif key==glfw.KEY_V:   #initialize orientation
            a = 0
            b = 0
            r = 0
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'학번-9-1', None,None)
    if not window:
        glfw.terminate()
        return
