...
int main()
{
	//obtain machine accuracy of "double" and "double" of your computer
	// Use routine machar()
	...
	machar(&ibeta, &it, &irnd, &ngrd, &machep,
		&negep, &iexp, &minexp, &maxexp, &eps,
		&epsneg, &xmin, &xmax);
	
	printf("ibeta: %d\nit: %d\nmachep: %d\n", ibeta, it, machep);
	printf("machine accuracy(double): %f\n", pow(ibeta, machep));
	printf("machine accuracy 0.17f: %.17f\n", pow(ibeta, machep));
